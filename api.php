...
$DB_PASS = ''; // 👉 ĐIỀN MẬT KHẨU DATABASE CỦA BẠN
...
switch ($action) {
    ...
    case 'join_room':     joinRoom(); break;
    case 'set_secret':    setSecret(); break;
    case 'submit_guess':  submitGuess(); break;
    case 'get_room':      getRoom(); break;
    case 'get_guesses':   getGuesses(); break;
    case 'cancel_room':   cancelRoom(); break;
    default:
        echo json_encode(['error' => 'Invalid action', 'actions' => [
            'create_room', 'join_room', 'set_secret', 'submit_guess', 'get_room', 'get_guesses', 'cancel_room'
        ]]);
}
...
function createRoom() {
    ...
    if (!$roomId) return error('Không thể tạo phòng, thử lại sau');
...
function joinRoom() {
    ...
    $playerId = getParam('player_id');
    if (!$roomId || !$playerId) return error('Missing room_id or player_id');

    $stmt = $pdo->prepare("SELECT * FROM rooms WHERE id = ?");
    $stmt->execute([$roomId]);
    $room = $stmt->fetch();

    if (!$room) return error('Không tìm thấy phòng ' . $roomId);
...
function setSecret() {
    ...
    if (!$roomId || !$playerId || !$secret) return error('Missing params');
    if (!preg_match('/^\d{4}$/', $secret)) return error('Secret phải là 4 chữ số');
    ...
    // Xác định player nào
    $playerKey = null;
    if ($room['player1_id'] === $playerId) $playerKey = 'player1_secret';
    elseif ($room['player2_id'] === $playerId) $playerKey = 'player2_secret';
...
function submitGuess() {
    ...
    if (!$roomId || !$playerId || !$digits) return error('Missing params');
    if (!preg_match('/^\d{4}$/', $digits)) return error('Digits phải là 4 chữ số');

    $stmt = $pdo->prepare("SELECT * FROM rooms WHERE id = ?");
    $stmt->execute([$roomId]);
    $room = $stmt->fetch();
    if (!$room) return error('Phòng không tồn tại');
    if ($room['status'] !== 'playing') return error('Game chưa bắt đầu');
